 ...
 *
 * Adding one opens a dialog so the panel does not grow a form. Secrets are
 * write-only — the API never returns a value.
 */
  ...
  return (
    <>
      <PanelHeader title="Connections">
        <PanelAction label="Add a connection" onClick={() => { setAdding(true); setError(null) }}>
      ...
      <PanelBody>
        <div className="px-2">
          ...
          {connections.map((c) => (
            <div key={c.id} className="group relative flex items-start gap-1.5 rounded-md hover:bg-accent/40">
              {/* The row opens the configuration. The remove button is
                  positioned over it rather than nested inside, since a button
                  cannot contain another button. */}
              <button
                type="button"
                onClick={() => setViewing(c)}
                title={`${c.name} — view configuration`}
                className="flex min-w-0 flex-1 items-start gap-2 px-2 py-1.5 text-left cursor-pointer"
              >
                <Plug className="h-3.5 w-3.5 mt-0.5 shrink-0 text-primary" />
          ...
          {/* A failed load used to render as "None yet.", which is how a
              workspace with two connections could look like one with none. */}
          {!isLoading && isError && (
          ...
          {!isLoading && !isError && connections.length === 0 && (
            <p className="px-2 py-1.5 text-xs text-muted-foreground">
              None yet. Add an API or MCP server this workspace&apos;s agent can reach.
            </p>
          )}
