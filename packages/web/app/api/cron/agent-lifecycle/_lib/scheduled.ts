...
import { getEnvForModel, resolveCliModel, ENDPOINT_MODEL_PREFIX, type Agent } from "@switchboard/common"
...
export async function startJobExecution(
  // Workspace is joined by the caller so a scheduled run resolves it exactly the
  // way an interactive chat does — cron and on-demand stay one code path.
  job: Prisma.ScheduledJobGetPayload<object> & { workspace: WorkspaceRuntime | null },
